...
import { submitCartData } from "@archetype-themes/scripts/utils/theme-request";
...
class BlockBuyButtons extends HTMLElement {
  ...
  async onButtonSubmit(event) {
    ...
    const formData = this.getFormDataWithSections(sectionsToBundle);
    const response = await submitCartData(formData);
